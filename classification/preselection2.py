    ...
    def transform(self, results_file=''):
        ...
        for i, index in enumerate(test_indexes):
            ...
            if images_indexes[index] in str(self.true_objects_indexes) and p:
                true_detected += 1
            if images_indexes[index] in str(self.false_objects_indexes) and not p:
                false_detected += 1
            else:
                wrong += 1

        print('TRUE DETECTED: ' + str(true_detected))
        print('FALSE DETECTED: ' + str(false_detected))
        ...
        plt.hist(train_images_indexes, bins=6)
        plt.show()
