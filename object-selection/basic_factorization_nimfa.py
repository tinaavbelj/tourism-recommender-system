...
class BasicFactorizationNmf:
    ...
    def cross_validation(self, k, parameters, mask, R12, results_file):
        """
        Makes k masks for cv
        ...
        for p_t1 in parameters:
            ...
            for current_cv_mask in cv_masks:
                ...
                nmf = nimfa.Nmf(V, max_iter=200, rank=p_t1, update='euclidean', objective='fro')
                nmf_fit = nmf()
        ...
        data = {'p_t1': all_p_t1,
                'score': all_scores}
        df = pd.DataFrame(data, columns=['p_t1', 'score'])
    ...
    def factorization(self, cv_results_file):
        ...
        nmf = nimfa.Nmf(V, max_iter=200, rank=best_p_t1, update='euclidean', objective='fro')
        nmf_fit = nmf()
